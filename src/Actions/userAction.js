...
export const register = (name, email, password) => async (dispatch) => {
    try {
        ...
        const config = { headers: { 'Content-Type': 'application/json' } }
        const { data } = await axios.post("http://localhost:4000/api/users/registration", { name, email, password }, config);
        dispatch({
...
export const getUserDetails = (id) => async (dispatch, getState) => {
    try {
        ...
        const { data } = await axios.get(`http://localhost:4000/api/users/profile/${id}`);
        dispatch({
